...
class _WriteState extends State<Write>{
  // 1. 텍스트 입력창에 사용되는 컨트롤러 객체 선언 , 목적 : 입력받은 값 가져오기 위해
  ...
  void todoSave() async{
    try{
      ...
      final response = await dio.post("http://localhost:8080/day04/todos" , data : sendData );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar( title : Text("할일 등록 화면"),), // Appbar : 뒤로가기 제공
      body: Center(
        child: Column(
          children: [
            ...
            Text("할일을 등록해") , // 텍스트 위젯
            SizedBox( height : 30, ) ,
            TextField( // title 입력받는 텍스트 입력 위젯
