...
class _DetailState extends State<Detail>{
  ...
  // (1) intState() 해당 위젯이 최초로 열렸을때 딱 1번 실행 하는 함수 ( 위젯 생명주기 ) , context 위젯트리를 제공 받을수 없다.
  // (2) dedChangeDependencies() 이전(부모) 위젯이 변경 되었을때 실행 되는 함수 ( 위젯 생명주기 ) , context 위젯트리를 제공 받는다.
  
  // *1. 이전 위젯으로 부터 매개변수로 받은 id 를 가져오기
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    int id = ModalRoute.of(context)!.settings.arguments as int; // 이전 위젯으로 부터 매개 변수로 받은 id를 가져오기
    todoFindById(id); // 가져온 id를 서버에게 보내서 정보를 요청하기.
  } // override end
  
  // *2. 자바통신 과 통신 하여 id 에 해당 하는 상세 정보 요청
  Dio dio = Dio();
  Map< String,dynamic > todo = { }; // 응답 결과를 저장하는 하나의 '일정' 객체 선언
  void todoFindById(id) async{
    try{
      final response = await dio.get("http://localhost:8080/day04/todos/view?id=$id");
      final data = response.data;
      setState(() {
        todo = data; // 등답받은 결과를 상태변수에 대입
      });
      print( todo );
    }catch(e){ print( e ); }
  }
