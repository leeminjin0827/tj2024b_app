...
class _UpdateState extends State<Update>{ // 클래스명 앞에 _언더바는 dart에서 private 키워드
  ...
  // 2
  Dio dio = Dio();
  Map<String , dynamic > todo = {}; // JSON 타입은 key은 무조건 문자일 그래서 String , value은 다양한 자료타입 이므로 dynamic(동적타입)
  ...
  // 완료 여부를 저장하는 상태 변수, 컨트롤러 없이 직접 렌더링
  bool done = true;
  ...
  void todoUpdate( ) async{
    try{
      ...
      final response = await dio.put("http://localhost:8080/day04/todos" , data: sendData );
      final data = response.data;
