...
class _ProductRegisterState extends State<ProductRegister>{
  ...
  final TextEditingController pcontentController = TextEditingController(); // 제품설멍
  final TextEditingController ppriceController = TextEditingController(); // 제품가격
  int? cno = 1; // 카테고리 번호 // 드롭다운
  final dio = Dio();
  final String baseUrl = "http://192.168.40.88:8080";
  List<dynamic> categoryList = []; // 카테고리 목록

  @override // 위젯이 로드될때 1번 실행 함수
  void initState() { onCategory(); }

  // 4. 이미지 피커 : 사용자의 파일을 플러터로 가져오기
    // 1. pubspec.yamal 파일의 디펜더시 추가 : image_picker: ^1.1.2
  List<XFile> selectedImage = []; // 현재 선택된 이미지들을 저장하는 리스트
  void onSelectImage() async { // 이미지 선택 이벤트 함수
    try{
      ImagePicker picker = ImagePicker(); // 이미지 파커 객체생성 , .pickMultiImage() 이미지 여러개 , .pickImage() 이미지 한개
      List<XFile> pickedFiles = await picker.pickMultiImage(); // 사용자가 선택한 이미지들을 XFILE 파일으로 반환한다.
        // XFile 인터페이스는 이미지들을 조작할 수 있는 메소드 제공한다.
      if( pickedFiles.isNotEmpty ){ // 선택한 이미지가 존재하면 상태변수에 저장한다.
  ...
  void onRegister() async {
    try{
      ...
      // formData.fields.add( MapEntry("필드명", "깂")); // 필드명은 자바의 DTO 필드명과 동일한다.
      formData.fields.add( MapEntry("pname", pnameController.text ) );        // 폼에 입력받은 제품명 전달
      formData.fields.add( MapEntry("pcontent" , pcontentController.text ));  // 폼에 입력받은 제품설명 전달
      formData.fields.add( MapEntry("pprice" , ppriceController.text ));      // 폼에 입력받은 제품가격 전달
      formData.fields.add( MapEntry("cno" , '${cno}' ) );                     // 폼에 입력받은 카테고리 번호 전달
      // (*) 현재 선택된 이미지들을 formData 에 담아주기.
      for( XFile xFile in selectedImage ){
        // Xfile 의 path : 파일경로 , Xfile 의 name : 파일이름 // dio의 multipartFile 이용하여 파일객체 만들기.
  ...
  Widget build(BuildContext context) {
    ...
    Widget CategoryDropdown(){
      ...
      // onChanged : 드론다운 값의 선택이 변경 되었을때 이벤트 발생함수.
      return DropdownButtonFormField<int>( // 제네릭 <int> : value 에 들어가는 값 타입
        value: cno,
        hint: Text("카테고리 선택"), // 가이드라인
        decoration: InputDecoration( border: OutlineInputBorder() ), // 바깥 데두리
        ...
        onChanged: ( value ) { setState(() { cno = value; }); } // 변경된/선택된 값을 cno 에 대입
      );
      ...
      return Container(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context , index){ // 반복문
            final XFile xFile = selectedImage[index]; // index 변째 xfile 꺼내기
            return Padding(
              padding: EdgeInsets.all(5), // 여백
              child: SizedBox(
                width: 100, height: 100, // 이미지 사이즈 설정
                child: Image.file( File( xFile.path )), // 현재 index번째의 xfile 경로를 출력
              ),
            );
          }
        ),
