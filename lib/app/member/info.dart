...
class _InfoState extends State<Info>{
  // 1. 상태 변수
  int mno = 0; // 30;
  String memail = ''; // "qwe@qwer.com";
  String mname = ''; // "강호동";
  // 2. 해당 페이지(위젯) 열었을때 실행되는 함수
  @override
  void initState() {
    loginCheck();
  } // initState end
  // 3. 로그인 상태를 확인 하는 함수
  bool? isLogin; // Dart 문법 중에 타입? 은 null 포함할 수 있다 뜻
  ...
  void onInfo( token ) async{
    try{
      ...
      final response = await dio.get("http://192.168.40.88:8080/member/info");
  ...
  void logout( ) async {
    ...
    Dio dio = Dio();
    dio.options.headers['Authorization'] = token;
    final response = await dio.get("http://192.168.40.88:8080/member/logout");
    // 전역변수(클라이언트) 에도 토큰 삭제
